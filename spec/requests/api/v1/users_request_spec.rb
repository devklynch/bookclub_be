...
RSpec.describe "User endpoints", type: :request do
  before :each do
    ...
    # Replace the default options with custom ones for @poll1
  ...
  describe "Show all club data for a user" do
    context "successful requests" do
      ...
      it "should include upcoming events with attendee status when available" do
        ...
        Attendee.create!(user: @user1, event: event, attending: true)
    ...
    context "edge cases" do
      ...
      it "should exclude past events from upcoming_events" do
      ...
      it "should exclude expired polls from active_polls" do
        expired_poll = create(:poll, 
          poll_question: "Expired Poll?", 
          expiration_date: Date.today - 1.week, 
          book_club: @book_club1
        )
        ...
        poll_questions = json[:data][:attributes][:active_polls].map { |p| p[:poll_question] }
        expect(poll_questions).not_to include("Expired Poll?")
      end
    ...
    context "authorization" do
      ...
      it "should give an error if user does not exist" do
        ...
        expect(json[:error]).to include("Couldn't find User")
      end
    end
